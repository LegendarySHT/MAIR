#!/usr/bin/env python
# ===- lib/xsan/scripts/xsan_symbolize.py -----------------------------------===#
#
...
"""
Example of use:
  xsan_symbolize.py -c "$HOME/opt/cross/bin/arm-linux-gnueabi-" -s "$HOME/SymbolFiles" < xsan.log
...
def fix_filename(file_name):
    ...
    file_name = re.sub(".*xsan_[a-z_]*.(cc|cpp):[0-9]*", "_xsan_rtl_", file_name)
...
class AsanSymbolizerPlugIn(object):
    """
    This is the interface the `xsan_symbolize.py` code uses to talk
    to plugins.
    """
