...
/* 
  ...
  TSan@LLVM20 masks the high 4-bit of the 48-bit VMA to map it into 
  the TSan's shadow memory. To avoid conflicts, the n in 48-bit address
  0x_n_________ could not overlap with other app areas. 

  In TSan, for 
    - LoAppMem,  i.e., main binary section,  n = [0, 0]
    - MidAppMem, i.e., pie binary section,   n = [5, a)
    - HiAppMem,  i.e., stack/module section, n = [a, f]
  Currently, in the 4-bit space [0, f], we have [1, 4] not used.
  Therefore, we select n = [2, 4) for the heap section and reserve 
  [1, 1] and [4, 4] for future use.

  Note that the heap allocator requires a address space to 
  be the power of 2, so we set it to 2T.
 */
/// FIXME: ASan@LLVM20 uses dynamic heap base address, is it Okay to still use 
/// the fixed heap base address?
// const uptr kAllocatorSpace = 0x720000000000ULL;
// const uptr kAllocatorSize  =  0x20000000000ULL;  // 2T.
const uptr kAllocatorSpace = __xsan::Mapping48AddressSpace::kHeapMemBeg;
const uptr kAllocatorSize = __xsan::Mapping48AddressSpace::kHeapMemEnd -
                            __xsan::Mapping48AddressSpace::kHeapMemBeg;  // 2T.
typedef DefaultSizeClassMap SizeClassMap;
