...
static XsanAllocator &get_allocator();
...
enum {
  // Either just allocated by underlying allocator, but XsanChunk is not yet
  // ready, or almost returned to undelying allocator and XsanChunk is already
  // meaningless.
  CHUNK_INVALID = 0,
...
class XsanChunk : public ChunkBase {
 public:
...
struct QuarantineCallback {
  ...
  void Recycle(XsanChunk *m) {
    void *p = get_allocator().GetBlockBegin(m);
    if (p != m) {
      // Clear the magic value, as allocator internals may overwrite the
      // contents of deallocated chunk, confusing GetXsanChunk lookup.
    ...
    PoisonShadow(m->Beg(), RoundUpTo(m->UsedSize(), ASAN_SHADOW_GRANULARITY),
                 kXsanHeapLeftRedzoneMagic);

    // Statistics.
    XsanStats &thread_stats = GetCurrentThreadStats();
...
typedef Quarantine<QuarantineCallback, XsanChunk> XsanQuarantine;
typedef XsanQuarantine::Cache QuarantineCache;

void XsanMapUnmapCallback::OnMap(uptr p, uptr size) const {
//   PoisonShadow(p, size, kXsanHeapLeftRedzoneMagic);
//   // Statistics.
//   XsanStats &thread_stats = GetCurrentThreadStats();
//   thread_stats.mmaps++;
...
void XsanMapUnmapCallback::OnUnmap(uptr p, uptr size) const {
...
AllocatorCache *GetAllocatorCache(XsanThreadLocalMallocStorage *ms) {
  CHECK(ms);
  return &ms->allocator_cache;
}

QuarantineCache *GetQuarantineCache(XsanThreadLocalMallocStorage *ms) {
  CHECK(ms);
...
struct Allocator {
  ...
  void RePoisonChunk(uptr chunk) {
    ...
    XsanChunk *ac = GetXsanChunk((void *)chunk);
    ...
    if (ac && atomic_load(&ac->chunk_state, memory_order_acquire) ==
      ...
      if (chunk < beg && beg < end && end <= chunk_end) {
        // Looks like a valid XsanChunk in use, poison redzones only.
        PoisonShadow(chunk, beg - chunk, kXsanHeapLeftRedzoneMagic);
        uptr end_aligned_down = RoundDownTo(end, ASAN_SHADOW_GRANULARITY);
        FastPoisonShadowPartialRightRedzone(
            end_aligned_down, end - end_aligned_down,
            chunk_end - end_aligned_down, kXsanHeapLeftRedzoneMagic);
        return;
      }
    }

    // This is either not an XsanChunk or freed or quarantined XsanChunk.
    // In either case, poison everything.
    PoisonShadow(chunk, allocated_size, kXsanHeapLeftRedzoneMagic);
  }
  ...
  XsanChunk *ChooseChunk(uptr addr, XsanChunk *left_chunk,
                         XsanChunk *right_chunk) {
    if (!left_chunk)
      return right_chunk;
    if (!right_chunk)
      return left_chunk;
    ...
    CHECK(XsanChunkView(left_chunk).AddrIsAtRight(addr, 1, &l_offset));
    CHECK(XsanChunkView(right_chunk).AddrIsAtLeft(addr, 1, &r_offset));
  ...
  bool UpdateAllocationStack(uptr addr, BufferedStackTrace *stack) {
    XsanChunk *m = GetXsanChunkByAddr(addr);
    if (!m) return false;
  ...
  void *Allocate(uptr size, uptr alignment, BufferedStackTrace *stack,
    ...
    if (*(u8 *)MEM_TO_SHADOW((uptr)allocated) == 0 && CanPoisonMemory()) {
      ...
      PoisonShadow((uptr)allocated, allocated_size, kXsanHeapLeftRedzoneMagic);
    }
    ...
    XsanChunk *m = reinterpret_cast<XsanChunk *>(chunk_beg);
  ...
  bool AtomicallySetQuarantineFlagIfAllocated(XsanChunk *m, void *ptr,
  ...
  void QuarantineChunk(XsanChunk *m, void *ptr, BufferedStackTrace *stack) {
    ...
    if (t) {
      XsanThreadLocalMallocStorage *ms = &t->malloc_storage();
  ...
  void CommitBack(XsanThreadLocalMallocStorage *ms, BufferedStackTrace *stack) {
    AllocatorCache *ac = GetAllocatorCache(ms);
  ...
  // Returns nullptr if XsanChunk is not yet initialized just after
  // get_allocator().Allocate(), or is being destroyed just before
  // get_allocator().Deallocate().
  XsanChunk *GetXsanChunk(void *alloc_beg) {
    if (!alloc_beg)
      return nullptr;
    XsanChunk *p = reinterpret_cast<LargeChunkHeader *>(alloc_beg)->Get();
    if (!p) {
      if (!allocator.FromPrimary(alloc_beg))
        return nullptr;
      p = reinterpret_cast<XsanChunk *>(alloc_beg);
    }
  ...
  XsanChunk *GetXsanChunkByAddr(uptr p) {
    void *alloc_beg = allocator.GetBlockBegin(reinterpret_cast<void *>(p));
    return GetXsanChunk(alloc_beg);
  }

  // Allocator must be locked when this function is called.
  XsanChunk *GetXsanChunkByAddrFastLocked(uptr p) {
    void *alloc_beg =
  ...
  uptr AllocationSize(uptr p) {
    XsanChunk *m = GetXsanChunkByAddr(p);
    if (!m) return 0;
    if (atomic_load(&m->chunk_state, memory_order_acquire) != CHUNK_ALLOCATED)
      return 0;
    if (m->Beg() != p) return 0;
  ...
  XsanChunkView FindHeapChunkByAddress(uptr addr) {
    XsanChunk *m1 = GetXsanChunkByAddr(addr);
    sptr offset = 0;
    if (!m1 || XsanChunkView(m1).AddrIsAtLeft(addr, 1, &offset)) {
      // The address is in the chunk's left redzone, so maybe it is actually
      // a right buffer overflow from the other chunk to the left.
      // Search a bit to the left to see if there is another chunk.
      XsanChunk *m2 = nullptr;
      for (uptr l = 1; l < GetPageSizeCached(); l++) {
        m2 = GetXsanChunkByAddr(addr - l);
        if (m2 == m1) continue;  // Still the same chunk.
        break;
      }
      if (m2 && XsanChunkView(m2).AddrIsAtRight(addr, 1, &offset))
        m1 = ChooseChunk(addr, m2, m1);
    }
    return XsanChunkView(m1);
  }
...
bool XsanChunkView::IsValid() const {
  return chunk_ && atomic_load(&chunk_->chunk_state, memory_order_relaxed) !=
                       CHUNK_INVALID;
}
bool XsanChunkView::IsAllocated() const {
  return chunk_ && atomic_load(&chunk_->chunk_state, memory_order_relaxed) ==
                       CHUNK_ALLOCATED;
}
bool XsanChunkView::IsQuarantined() const {
  return chunk_ && atomic_load(&chunk_->chunk_state, memory_order_relaxed) ==
                       CHUNK_QUARANTINE;
}
uptr XsanChunkView::Beg() const { return chunk_->Beg(); }
uptr XsanChunkView::End() const { return Beg() + UsedSize(); }
uptr XsanChunkView::UsedSize() const { return chunk_->UsedSize(); }
u32 XsanChunkView::UserRequestedAlignment() const {
  return Allocator::ComputeUserAlignment(chunk_->user_requested_alignment_log);
}

uptr XsanChunkView::AllocTid() const {
  u32 tid = 0;
  u32 stack = 0;
...
uptr XsanChunkView::FreeTid() const {
  if (!IsQuarantined())
    return kInvalidTid;
  u32 tid = 0;
  u32 stack = 0;
...
AllocType XsanChunkView::GetAllocType() const {
  return (AllocType)chunk_->alloc_type;
}

u32 XsanChunkView::GetAllocStackId() const {
  u32 tid = 0;
  u32 stack = 0;
  chunk_->GetAllocContext(tid, stack);
  return stack;
...
u32 XsanChunkView::GetFreeStackId() const {
  if (!IsQuarantined())
    return 0;
  u32 tid = 0;
  u32 stack = 0;
...
XsanChunkView FindHeapChunkByAllocBeg(uptr addr) {
  return XsanChunkView(instance.GetXsanChunk(reinterpret_cast<void*>(addr)));
}

void XsanThreadLocalMallocStorage::CommitBack() {
...
void *xsan_reallocarray(void *p, uptr nmemb, uptr size,
  ...
  return xsan_realloc(p, nmemb * size, stack);
}
...
namespace __lsan {
void LockAllocator() {
  __xsan::get_allocator().ForceLock();
}

void UnlockAllocator() {
  __xsan::get_allocator().ForceUnlock();
}

void GetAllocatorGlobalRange(uptr *begin, uptr *end) {
  *begin = (uptr)&__xsan::get_allocator();
  *end = *begin + sizeof(__xsan::get_allocator());
}

uptr PointsIntoChunk(void *p) {
  uptr addr = reinterpret_cast<uptr>(p);
  __xsan::XsanChunk *m = __xsan::instance.GetXsanChunkByAddrFastLocked(addr);
  if (!m || atomic_load(&m->chunk_state, memory_order_acquire) !=
                __xsan::CHUNK_ALLOCATED)
    return 0;
  uptr chunk = m->Beg();
...
uptr GetUserBegin(uptr chunk) {
  __xsan::XsanChunk *m = __xsan::instance.GetXsanChunkByAddrFastLocked(chunk);
  return m ? m->Beg() : 0;
}

LsanMetadata::LsanMetadata(uptr chunk) {
  metadata_ = chunk ? reinterpret_cast<void *>(chunk - __xsan::kChunkHeaderSize)
                    : nullptr;
}
...
bool LsanMetadata::allocated() const {
  if (!metadata_)
    return false;
  __xsan::XsanChunk *m = reinterpret_cast<__xsan::XsanChunk *>(metadata_);
...
void ForEachChunk(ForEachChunkCallback callback, void *arg) {
  __xsan::get_allocator().ForEachChunk(callback, arg);
}

IgnoreObjectResult IgnoreObjectLocked(const void *p) {
  uptr addr = reinterpret_cast<uptr>(p);
  __xsan::XsanChunk *m = __xsan::instance.GetXsanChunkByAddr(addr);
  if (!m ||
...
void GetAdditionalThreadContextPtrs(ThreadContextBase *tctx, void *ptrs) {
  ...
  // This is necessary to prevent false positive leaks due to the XsanThread
  ...
  __xsan::XsanThreadContext *atctx =
      reinterpret_cast<__xsan::XsanThreadContext *>(tctx);
  __xsan::XsanThread *xsan_thread = atctx->thread;
  ...
  uptr thread_arg = reinterpret_cast<uptr>(xsan_thread->get_arg());
  if (!thread_arg)
    return;
