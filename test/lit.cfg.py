...
# Setup config name.
config.name = 'XSan' + config.name_suffix
...
if config.xsan_dynamic:
  ...
  if platform.system() == 'Windows':
    ...
    xsan_dynamic_flags += ["-D_MT", "-D_DLL", "-Wl,-nodefaultlib:libcmt,-defaultlib:msvcrt,-defaultlib:oldnames"]
...
clang_asan_cflags = clang_asan_static_cflags + xsan_dynamic_flags
clang_asan_cxxflags = clang_asan_static_cxxflags + xsan_dynamic_flags

# Add win32-(static|dynamic)-xsan features to mark tests as passing or failing
...
if config.host_os == 'Windows' and config.xsan_dynamic:
