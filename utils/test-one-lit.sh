...
if [ $# -eq 1 ]; then
    file="$1"
    san=""
    for s in asan msan tsan ubsan; do
        if [[ "$file" == *"$s"* ]]; then
            san="$s"
            break
        fi
    ...
    if [ -z "$san" ]; then
        echo "Error: None of the keywords asan, msan, tsan, ubsan found in the file name."
        echo "Please make sure the file path contains one of the keywords, or provide the sanitizer type (asan/msan/tsan/ubsan) as the second argument."
        exit 1
    fi
elif [ $# -eq 2 ]; then
    file="$1"
    san="$2"
    if [[ ! "$san" =~ ^(asan|msan|tsan|ubsan)$ ]]; then
        echo "Error: The second argument must be one of asan, msan, tsan, or ubsan."
        exit 1
    fi
else
    echo "Usage: $0 <file> [asan|msan|tsan|ubsan]"
    exit 1
fi
